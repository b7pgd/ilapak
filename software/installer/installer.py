...
def ensure_node_js() -> str | None:
    """Mencari executable node.exe di System PATH atau lokasi standar Windows."""
...
def register_or_update_service(nssm_bin: str, service_name: str, app_path: str, app_dir: str, args: str = "") -> bool:
...
def run_deployment_pipeline(backend_service: str, frontend_service: str, backend_exe: Path, frontend_server: Path):
    total_steps = 8
    ...
    if not register_or_update_service(nssm_bin, frontend_service, node_bin, str(frontend_server.parent), args=node_args):
...
def main():
    ...
    if not frontend_matches:
        print_error("Next.js standalone build tidak ditemukan.")
        print_info("Expected path: .next\\standalone\\server.js")
        print_info("Pastikan project Next.js menggunakan output: 'standalone' dan sudah di-build.")
        handle_exit(False, "Next.js standalone server.js not found.")

    frontend_server = prompt_frontend_choice(frontend_matches, ROOT_DIR)
    ...
    run_deployment_pipeline(backend_service, frontend_service, backend_exe, frontend_server)
